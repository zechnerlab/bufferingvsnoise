...
function dropletStats = CalculateStatisticsTernary_NonEQ_Correlation(NMat, nA, v, V, numStates)


dropletStats.A1_Mu = NMat(:, :, 1);
dropletStats.A1_Var = NMat(:, :, 7);
dropletStats.B1_Mu = NMat(:, :, 2);
dropletStats.B1_Var = NMat(:, :, 14);
dropletStats.S1_Mu = NMat(:, :, 3);
dropletStats.S1_Var = NMat(:, :, 21);
