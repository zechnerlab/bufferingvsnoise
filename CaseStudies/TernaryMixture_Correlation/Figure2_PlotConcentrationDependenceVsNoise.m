...
% active molecular turnover. Components A and B are considered to have correlated fluctuations
...
for u=1:length(regionBIdx)
    ...
    for k = length(phiTotGridA):-1:1 %plot reverse because otherwise the background of the larger ellipse will overlap with the lower one
        ...
        if (~isempty(idx))
            ...
            fprintf('NoiseA1/NoiseTotA = %f (phi=%f)\n', NoiseReductionA1(plotIdx), phiTotGridA(plotIdx));
            fprintf('NoiseB1/NoiseTotB = %f (phi=%f)\n', NoiseReductionB1(plotIdx), phiTotB);
