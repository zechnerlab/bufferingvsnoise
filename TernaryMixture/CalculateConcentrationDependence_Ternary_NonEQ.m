function NMat = CalculateConcentrationDependence_Ternary_NonEQ(phiTotGridA, phiTotGridB, grid, chiAA, chiAB, chiBB, kT, v, nA, V, gamma, alpha, beta, k2A, k2B, c1, c2, plotTraces)
...
for j=1:length(phiTotGridA)
    for u=1:length(phiTotGridB)
        ...
        options = {};
        [gridF, N] = ode15s(@ActiveTernaryMixture, grid, N0, options, kT, v, chiAA, chiAB, chiBB, V, nA, alpha, beta, gamma, k1A, k1B, k2A, k2B, c1, c2);
        
        % Plotting
        if (plotTraces == 1)
            figure(1);
            subplot(2,3,1);
            plot(gridF, N(:, 1), gridF, N(:, 1) - sqrt(N(:, 8)), gridF, N(:, 1) + sqrt(N(:, 8)));
            ...
            subplot(2,3,5);
            plot(gridF, N(:, 4), gridF, N(:, 4) + sqrt(N(:, 32)), gridF, N(:, 3) - sqrt(N(:, 32)));
